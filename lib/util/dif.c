			 ...
			 uint32_t num_blocks, const struct spdk_dif_ctx *ctx)
{
	...
	if (ctx->block_size == ctx->guard_interval + ctx->md_size) {
		/* If DIF is contained in the first 8/16 bytes of metadata,
		 * copy and DIF generation cannot be done simultaneously.
		 * Hence, we do copy, rewind SGL, and then generate DIF.
		 */
		dst_iov = dst_sgl->iov;
		dst_iovcnt = dst_sgl->iovcnt;

		dif_disable_copy(src_sgl, dst_sgl, num_blocks, ctx);

		_dif_sgl_init(dst_sgl, dst_iov, dst_iovcnt);

		dif_generate_split(dst_sgl, num_blocks, ctx);
	} else if (_dif_sgl_is_bytes_multiple(src_sgl, ctx->block_size) &&
		   _dif_sgl_is_bytes_multiple(dst_sgl, ctx->block_size)) {
