...
struct map_page_cfg {
	...
	uint64_t size;
	uint64_t mask;
	uint64_t num_pages_per_gb;
};

/**
 * g_map_page_cfg can not be static because it is used in the inline function
 * spdk_mem_map_translate.
 */
struct map_page_cfg g_map_page_cfg = {
	.shift = SHIFT_2MB,
	.size = VALUE_2MB,
	.mask = MASK_2MB,
	.num_pages_per_gb = 1UL << (SHIFT_1GB - SHIFT_2MB),
};

#define MAP_PAGE_SHIFT			(g_map_page_cfg.shift)
#define MAP_PAGE_SIZE			(g_map_page_cfg.size)
#define MAP_PAGE_MASK			(g_map_page_cfg.mask)
#define MAP_NUM_PAGES_PER_GB	(g_map_page_cfg.num_pages_per_gb)

#define MAP_256TB_IDX(vfn_page)	((vfn_page) >> (SHIFT_1GB - MAP_PAGE_SHIFT))
#define MAP_1GB_IDX(vfn_page)	((vfn_page) & ((1ULL << (SHIFT_1GB - MAP_PAGE_SHIFT)) - 1))
#define MAP_PAGE_OFFSET(ptr)	(((uintptr_t)(ptr)) & MAP_PAGE_MASK)

/* Page is registered */
#define REG_MAP_REGISTERED	(1ULL << 62)

/* A notification region barrier. The page translation entry that's marked
...
/* Second-level map table indexed by bits [page_shift..29] of the virtual address.
 * Each entry contains the address translation or error for entries that haven't
 * been retrieved yet.
 */
struct map_1gb {
	struct map_page map[0];
	/**
	 * Page table space.
	 * Do not put any fields after this!
	 */
};

#define MAP_SIZE_OF_MAP_1GB (sizeof(struct map_1gb) + MAP_NUM_PAGES_PER_GB * sizeof(struct map_page))
...
mem_map_notify_walk(struct spdk_mem_map *map, enum spdk_mem_map_notify_action action)
{
	...
	for (idx_256tb = 0;
		...
		if (!map_1gb) {
			if (contig_start != UINT64_MAX) {
				...
				rc = map->ops.notify_cb(map->cb_ctx, map, action,
							(void *)contig_start,
							contig_end - contig_start + MAP_PAGE_SIZE);
		...
		for (idx_1gb = 0; idx_1gb < MAP_NUM_PAGES_PER_GB; idx_1gb++) {
			if ((map_1gb->map[idx_1gb].translation & REG_MAP_REGISTERED) &&
			    (contig_start == UINT64_MAX ||
			     (map_1gb->map[idx_1gb].translation & REG_MAP_NOTIFY_START) == 0)) {
				/* Rebuild the virtual address from the indexes */
				uint64_t vaddr = (idx_256tb << SHIFT_1GB) | (idx_1gb << MAP_PAGE_SHIFT);
...
err_unregister:
	 ...
	 */
	idx_256tb = MAP_256TB_IDX((contig_start >> MAP_PAGE_SHIFT) - 1);
	idx_1gb = MAP_1GB_IDX((contig_start >> MAP_PAGE_SHIFT) - 1);
	contig_start = UINT64_MAX;
	...
	for (; idx_256tb < SIZE_MAX; idx_256tb--) {
		...
		for (; idx_1gb < UINT64_MAX; idx_1gb--) {
			...
			uint64_t vaddr = (idx_256tb << SHIFT_1GB) | (idx_1gb << MAP_PAGE_SHIFT);
			if ((map_1gb->map[idx_1gb].translation & REG_MAP_REGISTERED) &&
			    (contig_end == UINT64_MAX || (map_1gb->map[idx_1gb].translation & REG_MAP_NOTIFY_START) == 0)) {
				...
				if (contig_end != UINT64_MAX) {
					if (map_1gb->map[idx_1gb].translation & REG_MAP_NOTIFY_START) {
						contig_start = vaddr;
					}
					/* End of of a virtually contiguous range */
					map->ops.notify_cb(map->cb_ctx, map,
		...
		idx_1gb = MAP_NUM_PAGES_PER_GB - 1;
	}
...
uint64_t
spdk_mem_map_get_page_size(void)
{
	return g_map_page_cfg.size;
}
...
spdk_mem_register(void *_vaddr, size_t len)
{
	...
	if (((uintptr_t)vaddr & MAP_PAGE_MASK) || (len & MAP_PAGE_MASK)) {
	...
	while (seg_len > 0) {
		...
		seg_vaddr += MAP_PAGE_SIZE;
		seg_len -= MAP_PAGE_SIZE;
	}

	seg_vaddr = vaddr;
	seg_len = 0;
	while (len > 0) {
		spdk_mem_map_set_translation(g_mem_reg_map, (uint64_t)vaddr, MAP_PAGE_SIZE,
...
spdk_mem_unregister(void *_vaddr, size_t len)
{
	...
	while (len > 0) {
		reg = spdk_mem_map_translate(g_mem_reg_map, (uint64_t)vaddr, NULL);
		spdk_mem_map_set_translation(g_mem_reg_map, (uint64_t)vaddr, MAP_PAGE_SIZE, 0);
...
mem_map_get_map_1gb(struct spdk_mem_map *map, uint64_t vfn_page)
{
	struct map_1gb *map_1gb;
	uint64_t idx_256tb = MAP_256TB_IDX(vfn_page);
	size_t i;
	...
	if (!map_1gb) {
		...
		map_1gb = map->map_256tb.map[idx_256tb];
		if (!map_1gb) {
			map_1gb = malloc(MAP_SIZE_OF_MAP_1GB);
			if (map_1gb) {
				/* initialize all entries to default translation */
				for (i = 0; i < MAP_NUM_PAGES_PER_GB; i++) {
					map_1gb->map[i].translation = map->default_translation;
				}
			     ...
			     uint64_t translation)
{
	...
	/* Only page-aligned registrations are supported */
	if (((uintptr_t)vaddr & MAP_PAGE_MASK) || (size & MAP_PAGE_MASK)) {
		DEBUG_PRINT("invalid %s parameters, vaddr=%" PRIu64 " len=%" PRIu64 "\n",
			    __func__, vaddr, size);
		return -EINVAL;
	}

	vfn_page = vaddr >> MAP_PAGE_SHIFT;

	while (size) {
		map_1gb = mem_map_get_map_1gb(map, vfn_page);
		if (!map_1gb) {
			DEBUG_PRINT("could not get %p map\n", (void *)vaddr);
			return -ENOMEM;
		}

		idx_1gb = MAP_1GB_IDX(vfn_page);
		map_page = &map_1gb->map[idx_1gb];
		map_page->translation = translation;
...
spdk_mem_map_translate(const struct spdk_mem_map *map, uint64_t vaddr, uint64_t *size)
{
	...
	const struct map_page *map_page;
	uint64_t idx_256tb;
	uint64_t idx_1gb;
	uint64_t vfn_page;
	uint64_t cur_size;
	...
	vfn_page = vaddr >> MAP_PAGE_SHIFT;
	idx_256tb = MAP_256TB_IDX(vfn_page);
	idx_1gb = MAP_1GB_IDX(vfn_page);
	...
	cur_size = MAP_PAGE_SIZE - MAP_PAGE_OFFSET(vaddr);
	map_page = &map_1gb->map[idx_1gb];
	if (size == NULL || map->ops.are_contiguous == NULL ||
	    map_page->translation == map->default_translation) {
		if (size != NULL) {
			*size = spdk_min(*size, cur_size);
		}
		return map_page->translation;
	}

	orig_translation = map_page->translation;
	...
	while (cur_size < *size) {
		...
		map_page = &map_1gb->map[idx_1gb];
		if (!map->ops.are_contiguous(prev_translation, map_page->translation)) {
			break;
		}

		cur_size += MAP_PAGE_SIZE;
		prev_translation = map_page->translation;
	}
...
void
mem_map_use_page_shift(uint32_t page_shift)
{
	g_map_page_cfg.shift = page_shift;
	g_map_page_cfg.size = 1UL << page_shift;
	g_map_page_cfg.mask = g_map_page_cfg.size - 1;
	g_map_page_cfg.num_pages_per_gb = 1UL << (SHIFT_1GB - page_shift);
}
