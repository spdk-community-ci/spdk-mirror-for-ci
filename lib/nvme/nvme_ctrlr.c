...
static void
nvme_ctrlr_io_qpair_opts_copy(const struct spdk_nvme_io_qpair_opts *src,
			      struct spdk_nvme_io_qpair_opts *dst, size_t opts_size_src)
{
	if (!opts_size_src) {
			       ...
			       size_t opts_size)
{
	...
	if (user_opts) {
		nvme_ctrlr_io_qpair_opts_copy(user_opts, &opts, spdk_min(opts.opts_size, opts_size));
