			 ...
			 struct spdk_nvme_probe_ctx *probe_ctx)
{
	...
	if (rc) {
		...
		TAILQ_INSERT_TAIL(&probe_ctx->aborted_ctxs.head, detach_ctx, link);
		    ...
		    spdk_nvme_remove_cb remove_cb)
{
	...
	TAILQ_INIT(&probe_ctx->aborted_ctxs.head);
}
...
spdk_nvme_probe_poll_async(struct spdk_nvme_probe_ctx *probe_ctx)
{
	...
	TAILQ_FOREACH_SAFE(detach_ctx, &probe_ctx->aborted_ctxs.head, link, detach_ctx_tmp) {
		...
		if (rc != -EAGAIN) {
			...
			TAILQ_REMOVE(&probe_ctx->aborted_ctxs.head, detach_ctx, link);
			free(detach_ctx);
		}
	}

	if (TAILQ_EMPTY(&probe_ctx->init_ctrlrs) && TAILQ_EMPTY(&probe_ctx->aborted_ctxs.head)) {
		nvme_robust_mutex_lock(&g_spdk_nvme_driver->lock);
