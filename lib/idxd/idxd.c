			    ...
			    spdk_idxd_req_cb cb_fn, void *cb_arg)
{
	...
	for (num_blocks_seg = spdk_bioviter_first(&iter, siov, siovcnt, diov, diovcnt,
			   ...
			   spdk_idxd_req_cb cb_fn, void *cb_arg)
{
	...
	for (num_blocks_seg = spdk_bioviter_first(&iter, siov, siovcnt, diov, diovcnt,
			      ctx->block_size, ctx->block_size - ctx->md_size, &src, &dst);
	     num_blocks_seg > 0;
	     num_blocks_seg = spdk_ioviter_next(&iter, &src, &dst)) {
		src_seg_addr = (uint64_t)src;
		src_seg_len = num_blocks_seg * ctx->block_size;
		dst_seg_addr = (uint64_t)dst;
