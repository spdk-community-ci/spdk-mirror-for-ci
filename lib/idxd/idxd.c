...
{
	if (flags == NULL) {
		SPDK_ERRLOG("Unsupported block size (%d) and metadata size (%d) values",
...
idxd_get_source_dif_flags(const struct spdk_dif_ctx *ctx, uint8_t *flags)
{
	...
	*flags = 0;
	...
	if (!(ctx->dif_flags & SPDK_DIF_FLAGS_APPTAG_CHECK))  {
		SPDK_ERRLOG("DIF Application Tag Check flag must be set.\n");
		return -EINVAL;
	}
			   ...
			   spdk_idxd_req_cb cb_fn, void *cb_arg)
{
	...
	uint8_t dif_flags, src_dif_flags;
	...
	for (i = 0; i < siovcnt; i++) {
		...
		rc = idxd_validate_dif_check_buf_align(ctx, len);
		if (rc) {
			goto error;
		}
		...
		while (len > 0 && num_blocks_left > 0) {
			...
			rc = idxd_validate_dif_check_buf_align(ctx, seg_len);
			if (rc) {
				goto error;
			}
			    ...
			    spdk_idxd_req_cb cb_fn, void *cb_arg)
{
	...
	for (i = 0; i < siovcnt; i++) {
		...
		/* Validate the memory buffer alignment */
		rc = idxd_validate_dif_insert_buf_align(ctx, src_len, dst_len);
		if (rc) {
			goto error;
		}
...
#define HW_STATUS_DIF_ERROR 0x9
...
spdk_idxd_process_events(struct spdk_idxd_io_channel *chan)
{
	...
	STAILQ_FOREACH_SAFE(op, &chan->ops_outstanding, link, tmp) {
		...
		case IDXD_OPCODE_DIF_CHECK:
			if (spdk_likely(op->hw.status == HW_STATUS_DIF_ERROR)) {
				status = -EIO;
			}
			break;
		}
