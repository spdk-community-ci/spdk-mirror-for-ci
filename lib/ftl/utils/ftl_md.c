...
static void
ftl_md_create_spdk_buf(struct ftl_md *md)
{
	md->shm_fd = -1;
	md->data = spdk_zmalloc(md->data_blocks * FTL_BLOCK_SIZE, FTL_BLOCK_SIZE, NULL,
				SPDK_ENV_NUMA_ID_ANY, SPDK_MALLOC_DMA);
}

static void
ftl_md_create_heap(struct ftl_md *md)
{
	md->shm_fd = -1;
	md->data = calloc(md->data_blocks, FTL_BLOCK_SIZE);
}
...
static void
ftl_md_create_shm(struct ftl_md *md, int flags)
{
...
read_write_blocks_cb(struct spdk_bdev_io *bdev_io, bool success, void *arg)
{
	...
	ftl_stats_bdev_io_completed(md->dev, get_bdev_io_ftl_stats_dev_type(md->dev, bdev_io), bdev_io);
	    ...
	    spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	if (desc == dev->nv_cache.bdev_desc) {
		return ftl_nv_cache_bdev_read_blocks_with_md(dev, desc, ch, buf,
...
{
	if (desc == dev->nv_cache.bdev_desc) {
		return ftl_nv_cache_bdev_write_blocks_with_md(dev, desc, ch, buf,
				offset_blocks, num_blocks, cb, cb_arg);
	} else {
...
read_write_blocks(void *_md)
{
	...
	case FTL_MD_OP_RESTORE:
		rc = read_blocks(md->dev, region->bdev_desc, region->ioch,
				 md->io.data, md->io.address, blocks,
				  ...
				  spdk_bdev_io_wait_cb retry_fn)
{
	...
	rc = write_blocks(md->dev, md->region->bdev_desc, md->region->ioch,
			  ctx->buffer, persist_entry_lba(md, ctx->start_entry),
			  md->region->entry_size * ctx->num_entries,
			      ...
			      spdk_bdev_io_wait_cb retry_fn)
{
	...
	rc = read_blocks(md->dev, md->region->bdev_desc, md->region->ioch,
			 ctx->buffer, persist_entry_lba(md, ctx->start_entry),
			 md->region->entry_size, read_entry_cb, ctx);
...
void
ftl_md_read_entry(struct ftl_md *md, uint64_t start_entry, void *buffer,
		  ftl_md_io_entry_cb cb, void *cb_arg,
		  struct ftl_md_io_entry_ctx *ctx)
{
	if (spdk_unlikely(0 == md->region->entry_size)) {
...
static int
pattern_prepare(struct ftl_md *md, int data_pattern)
{
...
void
ftl_md_clear(struct ftl_md *md, int data_pattern)
{
	if (has_mirror(md)) {
		struct ftl_md *md_mirror = ftl_md_get_mirror(md);
		...
		if (0 == io_init(md, FTL_MD_OP_CLEAR) && 0 == pattern_prepare(md, data_pattern)) {
			/* First persist the mirror */
			ftl_md_clear(md_mirror, data_pattern);
		} else {
			spdk_thread_send_msg(spdk_get_thread(), exception, md);
		}
		return;
