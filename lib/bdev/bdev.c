...
_bdev_io_complete(void *ctx)
{
	...
	/**
	 * Until now we will not use the channel of this bdev_io anymore, we can put the
	 * channel reference safely.
	 */
...
bdev_reset_complete(struct spdk_bdev *bdev, void *_ctx, int status)
{
	...
	while (!TAILQ_EMPTY(&queued_resets)) {
		reset_io = TAILQ_FIRST(&queued_resets);
		TAILQ_REMOVE(&queued_resets, reset_io, internal.link);
		reset_io->internal.status = bdev_io->internal.status;
		spdk_thread_send_msg(spdk_bdev_io_get_thread(reset_io),
				     bdev_io_complete, reset_io);
	}
