...
tgt2addr="$NVMF_FIRST_INITIATOR_IP"
ns1uuid=$(uuidgen)
ns2uuid=$(uuidgen)
ns3uuid=$(uuidgen)

rpc_cmd <<- CONFIG
	bdev_null_create null0 100 4096 -u "$ns1uuid"
	bdev_null_create null1 100 4096 -u "$ns2uuid"
...
"$rootdir/scripts/rpc.py" -s "$tgt2sock" <<- CONFIG
	...
	nvmf_subsystem_add_ns "$subnqn3" nvme0n1 -n 1
	nvmf_subsystem_add_ns "$subnqn3" nvme0n2 -n 2
