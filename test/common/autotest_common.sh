...
function get_config_params() {
	...
	if ((SPDK_TEST_VBDEV_COMPRESS == 0)); then
		# SPDK_TEST_VBDEV_COMPRESS set and used only on systems with QAT offload
		# installed; We can't run this anywhere.
		filter+=("with-dpdk-compressdev")
	fi

	if ((SPDK_TEST_FUZZER == 1 || SPDK_TEST_FUZZER_SHORT == 1)); then
		# See: https://github.com/spdk/spdk/issues/3121
		filter+=("with-ocf")
	fi
