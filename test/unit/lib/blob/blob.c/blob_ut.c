...
blob_write(void)
{
	...
	pages_per_cluster = 4 * spdk_bs_get_cluster_size(bs) / spdk_bs_get_page_size(bs);
...
blob_insert_cluster_msg_test(void)
{
	...
	/* Have extra 12KB space is to account for 16KB phys blocklen testing */
	struct {
		struct spdk_blob_md_page page;
		uint8_t __buf[4096 * 3];
	} md = {};
...
blob_io_unit(void)
{
	...
	bsopts.cluster_sz = g_phys_blocklen * 4;	/* 8 * 4 = 32 io_unit */
