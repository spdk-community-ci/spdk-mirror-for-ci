...
ae4dma_queue_full_check(void)
{
	...
	for (uint8_t i = 0; i < 28; i++) {
		CU_ASSERT(ae4dma_desc_cmdq_full(i) == 1)
	}
	CU_ASSERT(ae4dma_desc_cmdq_full(28) == 0)
	CU_ASSERT(ae4dma_desc_cmdq_full(29) == 0)
	CU_ASSERT(ae4dma_desc_cmdq_full(30) == 0)
}

static void
ae4dma_max_desc_check(void)
{
	/* Checking for max descriptors of a queue */
	CU_ASSERT(ae4dma_max_descriptors_per_queue(32) == 0);
	CU_ASSERT(ae4dma_max_descriptors_per_queue(30) == 1);
	CU_ASSERT(ae4dma_max_descriptors_per_queue(33) == 1);
...
ae4dma_max_queue_config_check(void)
{
	...
	for (uint8_t i = 0; i < 16; i++) {
		CU_ASSERT(ae4dma_max_queues_per_device(i) == 0);
	}
	CU_ASSERT(ae4dma_max_queues_per_device(17) == 1);
	CU_ASSERT(ae4dma_max_queues_per_device(-1) == 1);
}
...
main(int argc, char **argv)
{
	...
	CU_ADD_TEST(suite, ae4dma_max_desc_check);
