...
DEFINE_STUB(spdk_nvme_qpair_get_fd, int, (struct spdk_nvme_qpair *qpair), 0);
...
test_spdk_nvme_poll_group_add_remove(void)
{
	...
	group = spdk_nvme_poll_group_create(NULL, NULL);
	SPDK_CU_ASSERT_FATAL(group != NULL);
	CU_ASSERT(group->interrupts_enabled == false);
	CU_ASSERT(STAILQ_EMPTY(&group->tgroups));
	...
	CU_ASSERT(group->interrupts_enabled == true);
	...
	CU_ASSERT(STAILQ_EMPTY(&group->tgroups));
	CU_ASSERT(group->interrupts_enabled == true);
	group->interrupts_enabled = false;
