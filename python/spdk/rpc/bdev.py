...
def bdev_examine(client, name):
    ...
    necessary if `auto_examine` has been set to false using `bdev_set_options`. By default, `auto_examine` is true and bdev
...
def bdev_ocf_create(client, name, mode, cache_bdev_name, core_bdev_name, cache_line_size=None):
    ...
    in the [OCF documentation](https://open-cas.github.io/cache_configuration.html#cache-mode)
    Args:
...
def bdev_ocf_set_seqcutoff(client, name, policy, threshold=None, promotion_count=None):
    ...
    found in [OpenCAS documentation](https://open-cas.github.io/guide_tool_details.html#seq-cutoff).
    Args:
        name: Bdev name
...
def bdev_ocf_flush_start(client, name):
    """Start flushing OCF cache device. Automatic flushes of dirty data are managed by OCF cleaning policy settings. In addition to that,
    all dirty data is flushed to core device when there is an attempt to stop caching. On the other hand, this RPC call gives a
    possibility to flush dirty data manually when there is a need for it, e.g. to speed up the shutdown process when data hasn't been
    flushed for a long time. This RPC returns immediately, and flush is then being performed in the background. To see the status of
    flushing operation use bdev_ocf_flush_status.
    Args:
        name: Bdev name
    """
    params = dict()
...
def bdev_ocf_flush_status(client, name):
    """Get flush status of OCF cache device. Automatic flushes of dirty data are managed by OCF cleaning policy settings. In addition to
    that, all dirty data is flushed to core device when there is an attempt to stop caching. On the other hand, there is a possibility
    to flush dirty data manually when there is a need for it, e.g. to speed up the shutdown process when data hasn't been flushed for
    a long time. This RPC reports if such manual flush is still in progress and if the operation was successful. To start manual flush
    use bdev_ocf_flush_start.
    Args:
        name: Bdev name
    Returns:
...
def bdev_malloc_create(client, num_blocks, block_size, physical_block_size=None, name=None, uuid=None, optimal_io_boundary=None,
    ...
    """Construct [Malloc bdev](https://spdk.io/doc/bdev.html#bdev_config_malloc) The `dif_type` parameter can have 0, 1, 2, or 3, and
    controls the check of the guard tag and the reference tag. If the `dif_type` is 1, 2, or 3, the malloc bdev compares the guard tag
    to the CRC-16 computed over the block data. If the `dif_type` is 1 or 2, the malloc bdev compares the reference tag to the
    computed reference tag. The computed reference tag for the first block of the I/O is the `init_ref_tag` of the DIF context, and
    the computed reference tag is incremented for each subsequent block. If the `dif_type` is 3, the malloc bdev does not check the
    reference tag. The application tag is not checked by the malloc bdev because the current block device API does not expose it to
    the upper layer yet.
    Args:
        ...
        dif_pi_format: Protection infromation format. Parameter --dif-type needs to be set together. 0=16b Guard PI, 1=32b Guard PI, 2=64b
        Guard PI. Default=0.
    Returns:
        Name of created block device.
    """
...
def bdev_malloc_delete(client, name):
    """Delete [Malloc bdev](https://spdk.io/doc/bdev.html#bdev_config_malloc)
    Args:
        name: Bdev name
    """
    params = dict()
...
def bdev_null_create(client, num_blocks, block_size, name, physical_block_size=None, uuid=None, md_size=None, dif_type=None,
                     dif_is_head_of_md=None, dif_pi_format=None):
    """Construct [Null bdev](https://spdk.io/doc/bdev.html#bdev_config_null)
    Args:
        num_blocks: Number of blocks
...
def bdev_null_delete(client, name):
    """Delete [Null bdev](https://spdk.io/doc/bdev.html#bdev_config_null).
    Args:
        name: Bdev name
    """
    params = dict()
...
def bdev_null_resize(client, name, new_size):
    """Resize [Null bdev](https://spdk.io/doc/bdev.html#bdev_config_null).
    Args:
        name: Bdev name
...
def bdev_raid_set_options(client, process_window_size_kb=None, process_max_bandwidth_mb_sec=None):
    """Set options for bdev RAID. This RPC can be called at any time, but the new value will only take effect for new RAID bdevs. The
    `process_window_size_kb` parameter defines the size of the `window` (LBA range of the RAID bdev) in which a background process
    like rebuild performs its work. Any positive value is valid, but the value actually used by a RAID bdev can be adjusted to the
    size of the RAID bdev or the write unit size.
    Args:
...
def bdev_raid_add_base_bdev(client, base_bdev, raid_bdev):
    """Add base bdev to existing RAID bdev. The RAID bdev must have an empty base bdev slot. The bdev must be large enough and have the
    same block size and metadata format as the other base bdevs.
    Args:
...
def bdev_aio_create(client, filename, name, block_size=None, readonly=None, fallocate=None):
    """Construct [Linux AIO bdev](https://spdk.io/doc/bdev.html#bdev_config_aio).
    Args:
...
def bdev_aio_rescan(client, name):
    """Rescan the size of [Linux AIO bdev](https://spdk.io/doc/bdev.html#bdev_config_aio).
    Args:
        name: Bdev name
    """
    params = dict()
...
def bdev_aio_delete(client, name):
    """Delete [Linux AIO bdev](https://spdk.io/doc/bdev.html#bdev_config_aio).
    Args:
        name: Bdev name
    """
    params = dict()
...
def bdev_nvme_set_options(client, action_on_timeout=None, timeout_us=None, timeout_admin_us=None, keep_alive_timeout_ms=None,
    ...
    has been created. Parameters, `ctrlr_loss_timeout_sec`, `reconnect_delay_sec`, and `fast_io_fail_timeout_sec`, are for I/O error
    resiliency. They can be overridden if they are given by the RPC `bdev_nvme_attach_controller`. If reconnect_delay_sec is zero,
    ctrlr_loss_timeout_sec has to be zero. If reconnect_delay_sec is non-zero, ctrlr_loss_timeout_sec has to be -1 or not less than
    reconnect_delay_sec. This can be overridden by bdev_nvme_attach_controller. If ctrlr_loss_timeout_sec is zero, reconnect_delay_sec
    has to be zero. If ctrlr_loss_timeout_sec is -1, reconnect_delay_sec has to be non-zero. If ctrlr_loss_timeout_sec is not -1 or
    zero, reconnect_sec has to be non-zero and less than ctrlr_loss_timeout_sec. This can be overridden by
    bdev_nvme_attach_controller. If fast_io_fail_timeout_sec is not zero, it has to be not less than reconnect_delay_sec and less than
    ctrlr_loss_timeout_sec if ctrlr_loss_timeout_sec is not -1. This can be overridden by bdev_nvme_attach_controller.
    Args:
...
def bdev_nvme_attach_controller(client, name, trtype, traddr, adrfam=None, trsvcid=None, priority=None, subnqn=None, hostnqn=None,
    ...
    """Construct [NVMe bdev](https://spdk.io/doc/bdev.html#bdev_config_nvme). This RPC can also be used to add additional paths to an
    existing controller to enable multipathing. This is done by specifying the `name` parameter as an existing controller. When adding
    an additional path, the hostnqn, hostsvcid, hostaddr, prchk_reftag, and prchk_guard_arguments must not be specified and are
    assumed to have the same value as the existing path. The parameters, `ctrlr_loss_timeout_sec`, `reconnect_delay_sec`, and
    `fast_io_fail_timeout_sec`, are mutually dependent. If `reconnect_delay_sec` is non-zero, `ctrlr_loss_timeout_sec` has to be -1 or
    not less than `reconnect_delay_sec`. If `reconnect_delay_sec` is zero, `ctrlr_loss_timeout_sec` has to be zero. If
...
def bdev_nvme_detach_controller(client, name, trtype=None, traddr=None, adrfam=None, trsvcid=None, subnqn=None, hostaddr=None,
    ...
    the controller being detached and the associated bdevs being deleted. returns true if the controller and bdevs were successfully
    destroyed or the address was properly removed, false otherwise.
    Args:
...
def bdev_nvme_reset_controller(client, name, cntlid=None):
    """For non NVMe multipath, reset an NVMe controller whose name is given by the `name` parameter. For NVMe multipath, an NVMe bdev
    controller is created and it aggregates multiple NVMe controllers. The `name` parameter is an NVMe bdev controller name and the
    `cntlid` parameter is used to identify an NVMe controller in the NVMe bdev controller. Reset only one NVMe-oF controller if the
    `cntlid` parameter is specified, or all NVMe-oF controllers in an NVMe bdev controller if it is omitted. Returns true if the
    controller reset was successful, false otherwise.
    Args:
...
def bdev_nvme_enable_controller(client, name, cntlid=None):
    """For non NVMe multipath, enable an NVMe controller whose name is given by the `name` parameter. For NVMe multipath, an NVMe bdev
    controller is created and it aggregates multiple NVMe controllers. The `name` parameter is an NVMe bdev controller name and the
    `cntlid` parameter is used to identify an NVMe controller in the NVMe bdev controller. Enable only one NVMe-oF controller if the
    `cntlid` parameter is specified, or all NVMe-oF controllers in an NVMe bdev controller if it is omitted. Returns true if the
    controller enablement was successful or a controller was already enabled, false otherwise.
    Args:
        name: NVMe controller name (or NVMe bdev controller name for multipath)
        cntlid: NVMe controller ID (used as NVMe controller name for multipath)
    Returns:
...
def bdev_nvme_disable_controller(client, name, cntlid=None):
    """For non NVMe multipath, disable an NVMe controller whose name is given by the `name` parameter. For NVMe multipath, an NVMe bdev
    controller is created and it aggregates multiple NVMe controllers. The `name` parameter is an NVMe bdev controller name and the
    `cntlid` parameter is used to identify an NVMe controller in the NVMe bdev controller. Disable only one NVMe-oF controller if the
    `cntlid` parameter is specified, or all NVMe-oF controllers in an NVMe bdev controller if it is omitted. Returns true if the
    controller disablement was successful or a controller was already disabled, false otherwise.
    Args:
        name: NVMe controller name (or NVMe bdev controller name for multipath)
        cntlid: NVMe controller ID (used as NVMe controller name for multipath)
    Returns:
...
def bdev_nvme_start_discovery(client, name, trtype, traddr, adrfam=None, trsvcid=None, hostnqn=None, wait_for_attach=None,
    ...
    """Start a discovery service for the discovery subsystem of the specified transport ID. The discovery service will read the discovery
    log page for the specified discovery subsystem, and automatically attach to any subsystems found in the log page. When determining
    a controller name to use when attaching, it will use the 'name' parameter as a prefix, followed by a unique integer for that
    discovery service. If the discovery service identifies a subsystem that has been previously attached but is listed with a
    different path, it will use the same controller name as the previous entry, and connect as a multipath. When the discovery service
    sees that a subsystem entry has been removed from the log page, it will automatically detach from that controller as well. The
    'name' is also used to later stop the discovery service.
    Args:
...
def bdev_nvme_set_preferred_path(client, name, cntlid):
    """Set the preferred I/O path for an NVMe bdev in multipath mode. NOTE: This RPC does not support NVMe bdevs in failover mode.
    Args:
        name: Name of the NVMe bdev
...
def bdev_rbd_get_clusters_info(client, name=None):
    ...
    Returns:
        Returns the cluster info of the Rados Cluster name if provided. Otherwise, it returns the cluster info of every registered Raods
        Cluster name.
    """
    params = dict()
    if name is not None:
...
def bdev_rbd_create(client, pool_name, rbd_name, block_size, name=None, user_id=None, config=None, cluster_name=None, uuid=None):
    """Create [Ceph RBD](https://spdk.io/doc/bdev.html#bdev_config_rbd) bdev. This method is available only if SPDK was build with Ceph
    RBD support.
    Args:
...
def bdev_rbd_delete(client, name):
    """Delete [Ceph RBD](https://spdk.io/doc/bdev.html#bdev_config_rbd) bdev. This method is available only if SPDK was build with Ceph
    RBD support.
    Args:
...
def bdev_rbd_resize(client, name, new_size):
    """Resize [Ceph RBD](https://spdk.io/doc/bdev.html#bdev_config_rbd) bdev. This method is available only if SPDK was build with Ceph
    RBD support.
    Args:
...
def bdev_iscsi_create(client, name, url, initiator_iqn):
    """Connect to iSCSI target and create bdev backed by this connection. This method is available only if SPDK was build with iSCSI
    initiator support.
    Args:
        name: Name of block device
...
def bdev_iscsi_delete(client, name):
    """Delete iSCSI bdev and terminate connection to target. This method is available only if SPDK was built with iSCSI initiator
    support.
    Args:
...
def bdev_ftl_create(client, name, base_bdev, cache, core_mask=None, fast_shutdown=None, uuid=None, overprovisioning=None,
                    l2p_dram_limit=None):
    """Create FTL bdev. This RPC is subject to change.
    Args:
...
def bdev_ftl_load(client, name, base_bdev, cache, core_mask=None, fast_shutdown=None, uuid=None, overprovisioning=None,
                  l2p_dram_limit=None):
    """Loads FTL bdev. This RPC is subject to change.
    Args:
        name: Bdev name
        base_bdev: Name of the base device
        cache: Name of the cache device
...
def bdev_ftl_unload(client, name, fast_shutdown=None):
    """Unloads FTL bdev. This RPC is subject to change.
    Args:
        name: Bdev name
...
def bdev_ftl_delete(client, name, fast_shutdown=None):
    """Delete FTL bdev. This RPC is subject to change.
    Args:
        name: Bdev name
        fast_shutdown: When set FTL will minimize persisted data during deletion and rely on shared memory during next load
    """
...
def bdev_ftl_unmap(client, name, lba, num_blocks):
    """Unmap range of LBAs. This RPC is subject to change.
    Args:
        name: Bdev name
...
def bdev_ftl_get_stats(client, name):
    """Get IO statistics for FTL bdev This RPC is subject to change.
    Args:
        name: Bdev name
    Returns:
...
def bdev_get_bdevs(client, name=None, timeout=None):
    """Get information about block devices (bdevs). The user may specify no parameters in order to list all block devices, or a block
    device may be specified by name.  If a timeout is specified, the method will block until a bdev with a specified name appears or
    the timeout expires.  By default, the timeout is zero, meaning the method returns immediately whether the bdev exists or not.
    Args:
...
def bdev_get_iostat(client, name=None, per_channel=None):
    """Get I/O statistics of block devices (bdevs). The user may specify no parameters in order to list all block devices, or a block
    device may be specified by name.
    Args:
        name: Bdev name to query (if omitted, query all bdevs)
...
def bdev_reset_iostat(client, name=None, mode=None):
    ...
    The user may specify no parameters in order to reset I/O statistics for all block devices, or a block device may be specified by
    name.
    Args:
...
def bdev_enable_histogram(client, name, enable, opc=None):
...
def bdev_get_histogram(client, name):
    ...
    Args:
        name: Block device name
    Returns:
        Returns base64 encoded histogram, granularity of the histogram buckets, ticks per second
    """
    params = dict()
    params['name'] = name
...
def bdev_nvme_get_controller_health_info(client, name):
    ...
    Returns:
        containing information about health log of the NVMe  bdev controller.
    """
    params = dict()
    params['name'] = name
...
def bdev_daos_create(client, num_blocks, block_size, pool, cont, name, oclass=None, uuid=None):
    """Construct [DAOS bdev](https://spdk.io/doc/bdev.html#bdev_config_daos) To find more about various object classes please visit [DAOS
    documentation](https://github.com/daos-stack/daos/blob/master/src/object/README.md). Please note, that DAOS bdev module uses the
    same CLI flag notation as `dmg` and `daos` commands, for instance, `SX` or `EC_4P2G2` rather than in DAOS header file `OC_SX` or
    `OC_EC_4P2G2`.
    Args:
...
def bdev_daos_delete(client, name):
    """Delete [DAOS bdev](https://spdk.io/doc/bdev.html#bdev_config_daos)
    Args:
...
def bdev_daos_resize(client, name, new_size):
    """Resize [DAOS bdev](https://spdk.io/doc/bdev.html#bdev_config_daos).
    Args:
...
def bdev_nvme_start_mdns_discovery(client, name, svcname, hostnqn=None):
    ...
    TP-8009). The discovery service will listen for the mDNS discovery events from the Avahi-daemon and will connect to the newly
    learnt discovery controllers. Then the discovery service will automatically attach to any subsystem found in the discovery log
    page from the discovery controllers learnt using mDNS. When determining a controller name to use when attaching, it will use the
    'name' parameter as a prefix, followed by a unique integer assigned for each of the discovery controllers learnt through mDNS,
    followed by a unique integer for that discovery service. If the discovery service identifies a subsystem that has been previously
    attached but is listed with a different path, it will use the same controller name as the previous entry, and connect as a
    multipath. When the discovery service sees that a subsystem entry has been removed from the log page, it will automatically detach
    from that controller as well. The 'name' is also used to later stop the discovery service.
    Args:
