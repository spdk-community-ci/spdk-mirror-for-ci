...
Format: 'user:u1 secret:s1 muser:mu1 msecret:ms1,user:u2 secret:s2 muser:mu2 msecret:ms2'""")
    ...
    *** Mutually exclusive with --disable-chap ***""", action='store_true')
    ...
    p.add_argument('-f', '--adrfam', help='NVMe-oF transport adrfam: e.g., ipv4, ipv6, ib, fc, intra_host')
    p.add_argument('-s', '--trsvcid', help='NVMe-oF transport service id: e.g., a port number (required for RDMA or TCP)')
    p.add_argument("-k", "--secure-channel", help="""The connection to that discovery subsys requires a secure channel""", action="store_true")
    p.add_argument('-n', '--subnqn', help='Subsystem NQN')
    p.add_argument("-w", "--allow-any-host", help="Allow any host to connect (don't enforce allowed host NQN list)", action="store_true")
