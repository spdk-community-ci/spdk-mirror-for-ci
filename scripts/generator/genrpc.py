...
def $rpc_method($pparams):
    \"\"\"$rpc_description$rpc_params_description\"\"\"
...
def generate_rpcs(schema):
    ...
    for method in schema["methods"]:
        ...
        method_desc = "\n\t".join(wrap(text=method["description"], width=130, break_long_words=False)).expandtabs(4)
        desc = [
        ...
        rpc = rpc.replace("$rpc_description", method_desc)
    ...
    # with open(f"{rpc_folder}/rpc.py", mode="w+", encoding="utf-8") as message:
...
def generate_docs(schema):
    ...
    with open(f"{doc_folder}/jsonrpc.md", mode="w+", encoding="utf-8") as new_doc:
        new_doc.write(doc)
        print("... wrote jsonrpc.md")
