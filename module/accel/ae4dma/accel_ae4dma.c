...
static void
ae4dma_free_device(struct ae4dma_device *dev)
{
	pthread_mutex_lock(&g_ae4dma_mutex);
...
{
	struct spdk_ae4dma_chan *chan = arg;

	return spdk_ae4dma_process_events(chan) != 0 ? SPDK_POLLER_BUSY :
	       SPDK_POLLER_IDLE;
}
...
#if 0
static int
ae4dma_submit_fill(struct ae4dma_io_channel *ae4dma_ch, struct spdk_accel_task *task)
{
	if (spdk_unlikely(task->d.iovcnt != 1)) {
		return -EINVAL;
	}

	return spdk_ae4dma_build_fill(ae4dma_ch->ae4dma_ch, task, ae4dma_done,
...
ae4dma_submit_copy(struct ae4dma_io_channel *ae4dma_ch, struct spdk_accel_task *task)
{
	...
	return spdk_ae4dma_build_copy(ae4dma_ch->ae4dma_ch, task, ae4dma_done,
...
ae4dma_submit_tasks(struct spdk_io_channel *ch, struct spdk_accel_task *accel_task)
{
...
#if 0
		case SPDK_ACCEL_OPC_FILL:
			rc = ae4dma_submit_fill(ae4dma_ch,  accel_task);
			break;
#endif
		case SPDK_ACCEL_OPC_COPY:
	...
	spdk_ae4dma_flush(ae4dma_ch->ae4dma_ch);
...
ae4dma_create_cb(void *io_device, void *ctx_buf)
{
	...
	ae4dma_dev = ae4dma_allocate_device();
	if (ae4dma_dev == NULL) {
		return -1;
	}
	...
	ch->poller = SPDK_POLLER_REGISTER(ae4dma_poll, ch->ae4dma_ch, 0);
...
ae4dma_destroy_cb(void *io_device, void *ctx_buf)
{
	...
	ae4dma_free_device(ch->ae4dma_dev);
