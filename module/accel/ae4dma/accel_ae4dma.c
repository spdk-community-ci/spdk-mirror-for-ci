...
#include "./lib/ae4dma/ae4dma_internal.h"
...
ae4dma_alloc_dev_channel(void)
{
	...
	pthread_mutex_unlock(&g_ae4dma_mutex);
	SPDK_ERRLOG("no more ae4dma channels are available\n");
	return NULL;
}
...
ae4dma_poll(void *arg)
{
	...
	return spdk_ae4dma_process_events(chan->ae4dma_ch, chan->ae4dma_chan_id) != 0 ? SPDK_POLLER_BUSY :
	       SPDK_POLLER_IDLE;
}
...
attach_cb(void *cb_ctx, struct spdk_pci_device *pci_dev, struct spdk_ae4dma_chan *ae4dma)
{
	...
	dev->ae4dma = ae4dma;
	dev->hwq_avail = ae4dma->cmd_q_count;//AE4DMA_MAX_CHANNELS
