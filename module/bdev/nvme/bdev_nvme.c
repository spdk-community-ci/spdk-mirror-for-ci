...
bdev_nvme_create_qpair(struct nvme_qpair *nvme_qpair)
{
	...
	if (spdk_interrupt_mode_is_enabled()) {
		spdk_fd_group_get_default_event_handler_opts(&hdlr_opts, sizeof(hdlr_opts));
		hdlr_opts.events = SPDK_INTERRUPT_EVENT_IN;
		hdlr_opts.fd_type = SPDK_FD_TYPE_VFIO;

		rc = spdk_nvme_poll_group_add_qpair_fd_ext(qpair, bdev_nvme_qpair_process_completion,
