...
_comp_submit_unmap_split(void *ctx)
{
	...
	uint64_t offset_blocks, num_blocks, optimal_io_boundary;

	assert(params->chunk_size % params->logical_block_size == 0);
	optimal_io_boundary = params->chunk_size / params->logical_block_size;

	offset_blocks = split_io->current_offset_blocks;
	num_blocks = optimal_io_boundary - offset_blocks % optimal_io_boundary;
	num_blocks = spdk_min(optimal_io_boundary, split_io->remaining_num_blocks);
