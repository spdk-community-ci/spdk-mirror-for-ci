...
nvme_init_ns_worker_ctx(struct ns_worker_ctx *ns_ctx)
{
	...
	struct spdk_nvme_poll_group *group;
	struct spdk_nvme_qpair *qpair;
	struct spdk_nvme_poll_group_opts pg_opts = {};
	...
	opts.delay_cmd_submit = ns_ctx->interrupt_mode ? false : true;
	opts.create_only = true;
	if (ns_ctx->interrupt_mode) {
		opts.event_cb_fn = nvme_process_io;
		opts.event_cb_arg = NULL;
	}
	...
	spdk_nvme_poll_group_default_opts(&pg_opts, sizeof(pg_opts));
	pg_opts.create_fd_group = ns_ctx->interrupt_mode ? true : false;

	ns_ctx->u.nvme.group = spdk_nvme_poll_group_create_ext(ns_ctx, NULL, &pg_opts);
